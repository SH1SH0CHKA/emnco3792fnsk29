...
class FrogAccountManager {
  static getAccounts() {
    if (fs.existsSync(path.join(__appData, "light_accounts.json"))) {
      return JSON.parse(
        fs.readFileSync(path.join(__appData, "light_accounts.json"))
      );
    } else {
  ...
  static saveAccounts(accounts) {
    fs.writeFileSync(
      path.join(__appData, "light_accounts.json"),
  ...
  static resetAccountsList() {
    fs.writeFileSync(path.join(__appData, "light_accounts.json"), "[]");
